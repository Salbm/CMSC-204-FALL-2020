...
public class ChatServer  implements Runnable{ 
  ...
  private static HashSet<PrintWriter> writers = new HashSet<PrintWriter>();
	...
	//@Override
	public void run() {
      ServerSocket listnr = null;
		try {
			//TODO STUDENT: create a server socket
			listnr = new ServerSocket(CHAT_ROOM_PORT);
			System.out.println("The chat server is running.");
          while (true) {	
          	//TODO STUDENT: listen for a client to join, then setup input and output channels
          	clientSocket= listnr.accept();
          	InputStream inStrm = clientSocket.getInputStream();
          	OutputStream outStrm = clientSocket.getOutputStream();
          	out=new PrintWriter(outStrm);
          	Reader readIn=new InputStreamReader(inStrm);
